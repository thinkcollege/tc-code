<?php
/**
 * @version		$Id: install.php 799 2011-01-11 15:43:07Z stian $
 * @category	Nooku
			...
			<td width="100%">
				<table class="adminlist">
					...
					<tbody id="tasks">
						...
						<tr class="row1">
							<td class="key hasTip" title="<?php echo JText::_('MySQL Version') ?>::<?php echo sprintf($versiontext, 'MySQL v5.0.41', $database->getVersion()) ?>"><?php echo JText::_('MySQL Version') ?></td>
							<td>
								<?php echo version_compare($database->getVersion(), '5.0.41', '>=')
								? '<strong>'.JText::_('OK').'</strong> - '.$database->getVersion()
								: sprintf($versiontext, 'MySQL v5.0.41', $database->getVersion()) ?>
							</td>
						</tr>
						<tr class="row0">
