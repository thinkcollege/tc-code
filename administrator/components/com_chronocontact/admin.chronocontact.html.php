...
class HTML_ChronoContact {
    ...
    ?>
	  ...
	  <script language="javascript" type="text/javascript">
		...
		function Checkform(){
				...
				<?php //if ($paramsvalues->get('parsehtml', '1') == '1') { ?>
  ...
  <td valign="top">		
   ...
   <td><img src='<?php echo $mainframe->getSiteURL(); ?>components/com_chronocontact/chrono_verification.php<?php if($paramsvalues->get('imtype') == '1'){ echo '?imtype=1';}else{ echo '?imtype=0';} ?>'></td>
	...
	<div id="page-plugins">
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'Plugins Settings' ); ?>
...
<form action="index2.php" method="post" name="adminForm" onSubmit="return Checkform()">
	...
	function showLicenseStatus(){
		...
		$configs = JComponentHelper::getParams('com_chronocontact');
		$valid = false;
		if($configs->get('licensecode', '') && !$configs->get('licensevalid', '')){
			$valid = HTML_ChronoContact::validateLicense($configs->get('licensecode', ''));
		}
		if($configs->get('licensevalid', '')){
			$valid = 'valid';
		}
		if($configs->get('licensecode', '') && (trim($valid) == 'valid')){
			if(!$configs->get('licensevalid', '')){
				$query     = "SELECT * FROM `#__components` WHERE `option` = 'com_chronocontact' AND parent='0'";
				$database->setQuery( $query );
            ...
            <?php
				...
				//print_r($matches);
			?>
            <span class="cf_alert">Your Chronoforms install at <?php echo $matches[2]; ?> is <strong>Licensed</strong></span>
            <?php
		}else{
			?>
            <style type="text/css">
            ...
            <?php
				...
				//print_r($matches);
			?>
            <span class="cf_alert">Your Chronoforms install at <?php echo $matches[2]; ?> is <strong>NOT Licensed</strong>, No limited features but for a small fee you get link free forms and help us continue the development and support</span>
            <?php
		}
	}
	...
	function validateLicense($licensekey){
		...
		if (!function_exists('curl_init')){ 
            ...
            <?php
				...
				//print_r($matches);
			?>
            <span class="cf_alert">Your Chronoforms license can Not be validated, please contact us <a href="http://www.chronoengine.com/contactus.html">here</a> Or at this email address webmaster@chronoengine.com</span>
			<?php
		}else{	
			$fields = ''; 
			$ch = curl_init();
