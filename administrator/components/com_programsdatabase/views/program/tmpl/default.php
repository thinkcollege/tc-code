...
<form onSubmit="validator();" action="./" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
<?php
$group	= '';
foreach ($this->questions as $q) {
	...
	if ($q->inputLabel) {
		print "<label for=\"$id\">$q->inputLabel</label><br />";
	}
	switch ($q->typeId) {
		...
		case 4:
			...
			$cVal		= 1;
			foreach ($choices as $c) { // test statement print "Val: $val";  print " Cval: " . $cVal; print " C: " .$c;
...
$xx = 97;
		...
		break;	
		case 7:
			print "<input type=\"file\" id=\"$id\" name=\"$id\" /> Current: <a href=\"./components/"
					. strtolower(JRequest::getWord('option')) . "/files/".strtolower($val)."\">$val</a> "
					. "<input type=\"checkbox\" id=\"$id-d\" name=\"{$id}[delete]\" value=\"1\" /> "
					. "<label for=\"$name-d\">Remove</label>";
		break;
	}
	
}  ?>
 <input type="hidden" name="option" value="com_programsdatabase" /><?php if(isset($this->program->id)): ?>
