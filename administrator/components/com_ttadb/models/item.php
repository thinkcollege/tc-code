...
//		if (!$tItem->bind(array('id' => JRequest::getInt('cid'), 'typeId' => $typeId, 'published' => 0)) || !$tItem->check()) {
			// Hack **  to get published radio button to work in item form
		if (!$tItem->bind(array('id' => JRequest::getInt('cid'), 'typeId' => $typeId, 'published' => JRequest::getInt('published'))) || !$tItem->check()) {
		...
		foreach ($attrs as $a) {
			...
			// Hack **  --in editing mode you couldn't uncheck values, they stayed checked. This solution will add some empty values to the Values table, though.
			 /*
				...
				$value['attrOfId']	= $a->attrOfId;  */
					...
					for ($i = 0; $i < ((count($ids[$a->attrOfId]) >= count($values["a{$a->attrOfId}"])) ? count($ids[$a->attrOfId]) : count($values["a{$a->attrOfId}"] )) ; $i++) { 
				
				unset($value['value']);
				$row				=& $this->getTable('Value');
				$value['id']		= isset($ids[$a->attrOfId][$i]) ? intval($ids[$a->attrOfId][$i]->id) : 0;
				$value['value']		= isset($values["a$a->attrOfId"][$i]) ?  $values["a$a->attrOfId"][$i] : (isset($ids[$a->attrOfId][$i])  ? $ids["a$a->attrOfId"][$i] : null);
				$value['count']		= $i;
				$value['attrOfId']	= $a->attrOfId; 
			// end hack
				
				if ($value['id'] > 0 && is_array($value['value']) && !empty($value['value']['delete'])) {
				// Hack **  deleting of files was not working	
				//	$row->_delete = true;
					$row->delete($value['id']);
					// end hack
				} 
				// Hack ** In editing mode the form was not removing empty values
		...
		if ($ret) {
			for ($i = 0; $i < count($trans); $i++) {
				...
				if (!empty($trans[$i]->_delete)) {
					...
					JError::raiseWarning(500, 'Failed to store data ' . $trans[$i]->id . ' with value "' . $trans[$i]->value . '" with error: ' . $trans[$i]->getError());
					#print "<p style=\"color:red;\">Failed to store data {$trans[$i]->id} with value {$trans[$i]->value} with error: {$trans[$i]->getError()}</p>";
					$ret = false;
					break;
				}
			}
