...
class TtaDBModelItems extends JModel {
	...
	function __construct() {
		...
		// Hack **  limitstart needs to be set so users aren't sent back to start of list after editing
//		$limitstart = JRequest::getVar('limitstart', 0, '', 'int');
		$typeId = JRequest::getInt('typeId');
		if($typeId == 110) {
		$limitstart = $mainframe->getUserStateFromRequest($option.'limitstart', 'limitstart', 0, 'int'); }
		else { 	$limitstart = JRequest::getVar('limitstart', 0, '', 'int'); }
	...
	protected function _buildQuery($typeId, $limit) {
		...
		// Hack ** --to make literature items list alphabetically
		...
		// $query	= " SELECT $t.`id` {$clauses['cols']} $title, $t.`typeId`, $t.`published`\n FROM {$clauses['tables']} WHERE $where ORDER BY `t$typeId-1`.`id`, `t$typeId-1`.`count`";
			if (($attr->typeId == 110) && ($task == 'listItems')) {
				$query	= " SELECT $t.`id` {$clauses['cols']} $title, $t.`typeId`, $t.`published`\n FROM {$clauses['tables']} WHERE `a41` != '' ORDER BY `a41`,`t$typeId-1`.`id`, `t$typeId-1`.`count`";
			}	elseif (($attr->typeId == 112) && ($task == 'listItems')) {
				$query	= " SELECT $t.`id` {$clauses['cols']} $title, $t.`typeId`, $t.`published`\n FROM {$clauses['tables']} WHERE `a61` != '' ORDER BY `a66` DESC,`t$typeId-1`.`id`";
			}
	...
	function getData($typeId = null, $limit = true) {
		...
		if (empty(self::$items[$typeId])) {
			...
			for ($i = 0; $i < count($results); $i++) {
				...
				foreach ($row as $label => $datum) {
					...
					//  PF --this is making checkbox choices invisible to users
				//	$parts[0] .= '-ids';
				$parts[0] .= '';
		...
		// Hack **  since this function is used by item we can't set limitstart or it blanks out item edit
//		return $typeId == JRequest::getInt('typeId') ? array_slice($ret, $this->getState('limitstart'), $this->getState('limit')) : $ret;
		
		$task = JRequest::getVar('task'); 
		$typeId = JRequest::getInt('typeId'); 
		if(($task !== 'editItem') && ($task !== 'addItem')) {
		return $typeId == JRequest::getInt('typeId') ? array_slice($ret, $this->getState('limitstart'), $this->getState('limit')) : $ret; } 
		 else { return $ret;}
		// end hack
...
//			if ((is_object($item) && is_array($item->{$attr}[0])) || is_array($item[$attr][0])) {
				// Hack **  file upload field was not working
			if (is_object($item)) {
				$datum = is_object($item) ? $item->{$attr} : $item[$attr];
				for ($i = 0; $i < count($datum[$i]); $i++) {
					if ($this->filterItem($datum[$i], $filter, $attrs[$attr]->attrs)) {
						return true;
					}
				}
				return false;
			} else if ($attrs[$attr]->typeId < 3) {
				$strs = array();
				$i = 0;
				while (strpos($filter[0], '"', $i) !== false) {
					$p2 = strpos($filte[0], '"', $i + 1);
					if ($p2 !== false) {
						$strs[] = substr($filter[0], $i + 1, $p2 - 1);			// get quoted string
						$filter[0] = substr($filter[0], 0, $i) . substr($filer[0], $p2 + 1);	// chop out quoted string from filter.
					} else {
						break;
					}
				}
				...
				foreach ($strs as $str) {
					if (substr_count(is_object($item) ? $item->{$attr} : $item[$attr], $str) > 0) {
						return true;
					}
				}
			} else {
