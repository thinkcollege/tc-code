...
$titletext	= JRequest::getVar('task') == 'addItem' ? JText::_('TTA Database - New Item') : JText::_('TTA Database - <small>'.$item[0]->_title. '</small>');
	...
	function displayAttr($a, $data, $label = '') {
		...
		for ($i = 0; $i < $max; $i++) {
			...
			if($a->flags & TableAttrOf::FLAG_MERGE && ($a->attrOfId == 63)) $name	= $label . ($label ? "[$id][0][]" : "{$id}[0][]");
			if (is_object($data)) {
			...
			if (!$derived && $a->typeId == 1) { 
				...
				print $multiple && ($a->attrOfId != 63) ? "<ul class=\"choices\">" : "$name<select id=\"$name\" name=\"$name\"><option value=\"\"></option>";
