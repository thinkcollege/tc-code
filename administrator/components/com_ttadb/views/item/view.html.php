...
$titletext	= JRequest::getVar('task') == 'addItem' ? JText::_('TTA Database - New Item') : JText::_('TTA Database - <small>'.$item[0]->_title. '</small>');
	...
	function displayAttr($a, $data, $label = '') {
		...
		if($multiple && ($a->attrOfId == 63) && ($a->flags & TableAttrOf::FLAG_MERGE) != 0 ) $max = max(5, count($data->$id) + 1);
		for ($i = 0; $i < $max; $i++) {
			$name	= $a->flags & TableAttrOf::FLAG_MERGE ? $label . ($label ? "[$id][]" : "{$id}[]") : $label . ($label ? "[$id][$i]" : "{$id}[$i]");
			if($a->flags & TableAttrOf::FLAG_MERGE && ($a->attrOfId == 63)) $name	= $label . ($label ? "[$id][$i]" : "{$id}[$i]");
			if (is_object($data)) {
			...
			if (!$derived && $a->typeId == 1) {  $editor =& JFactory::getEditor(); 

// Hack ** this displays a text area with the wysiwyg print "<textarea name=\"$name\" id=\"$name\" rows=\"4\" cols=\"45\">$val</textarea>" . (!$derived ? '</p>' : '');

echo $editor->display($name,$val,'100%','300','20','20','0'). (!$derived ? '</p>' : ''); 
				...
				print "<input type=\"text\" name=\"$name\" value=\"$val\" />" . (!$derived ? '</p>' : '');
			}  else {
				...
				foreach ($choices as &$c) {
					...
					if ($multiple && ($a->attrOfId != 63)) {
						...
						$selected = ($cVal == $val[$i]) ? ' selected="selected"' : ''; 
						print "<option value=\"$cVal\"$selected>$cLabel</option>";
					}
				}
				print $multiple && ($a->attrOfId != 63) ? "</ul>" : "</select>";
			}
			$sep = "<br />";
		}
		if (!$derived) {
