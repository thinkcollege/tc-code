...
* @version $Id: com_glossary.php 165 2011-10-18 00:37:16Z guilleva $
* @package Xmap
* @license GNU/GPL
...
class xmap_com_glossary {
	...
	*/
	...
	function getTree ( &$xmap, &$parent, &$params ) {
		if (!xmap_com_glossary::loadGlossaryApi()) {
		    return false;
		}
	...
	function getGlossaryTree( &$xmap, &$parent, &$params, $glossaryid ) 
	{
		$database =& JFactory::getDBO();
			...
			foreach ( $letters as $letter ) {
				...
				$node->priority = $params['letter_priority'];
				$node->changefreq = $params['letter_changefreq'];
				$node->link = 'index.php?option=com_glossary&task=list'.(_GLOSSARY_VERSION < '2.7'? '&glossid='.$glossaryid : '').'&letter='.$letter->tletter;
				$node->expandible = true;
	...
	function getLetterTree( &$xmap, &$parent, &$params,$letter,$glossaryid ) 
	{
		...
		if ( $letter && $glossaryid ) {
			...
			foreach($entries as $entry) {
				...
				$node->link = 'index.php?option=com_glossary'.(_GLOSSARY_VERSION >= '2.7'? '&letter='.$letter : '').'&id='.$entry->id;
				$node->expandible = false;
				$xmap->printNode($node);
			}
			$xmap->changeLevel(-1);
	...
	function loadGlossaryApi() {
	    if (class_exists('cmsapiConfiguration'))
	        return true;

        if (file_exists((JPATH_SITE.DS.'components'.DS.'com_glossary'.DS.'cms_is_joomla.php'))) {
    		require_once JPATH_SITE.DS.'components'.DS.'com_glossary'.DS.'cms_is_joomla.php';
    		require_once _CMSAPI_ABSOLUTE_PATH.'/components/com_glossary/glossary.autoload.php';
    		return true;
    	} elseif (file_exists((JPATH_SITE.DS.'components'.DS.'com_glossary'.DS.'cmsapi.interface.php'))){
