...
class TtaDBModelItems extends JModel {
...
} elseif ($attr->typeId == 110 && $task == 'search' && $refine == 'popular') 
	 {	$query	= " SELECT $t.`id` {$clauses['cols']} $title, $t.`typeId`, $t.`published`, $t.`rating`, $t.`updated`\n FROM {$clauses['tables']} WHERE $where ORDER BY `t110-1`.`rating` DESC,`t$typeId-1`.`id`";
} elseif ($attr->typeId == 110 && $task == 'search' && $refine == 'recent') 
	 {	$query	= " SELECT $t.`id` {$clauses['cols']} $title, $t.`typeId`, $t.`published`, $t.`rating`, $t.`updated`\n FROM {$clauses['tables']} WHERE $where ORDER BY $t.`updated` DESC,`t$typeId-1`.`id`";
}

else {$query	= " SELECT $t.`id` {$clauses['cols']} $title, $t.`typeId`, $t.`published`, $t.`rating`, $t.`updated`\n FROM {$clauses['tables']} WHERE $where ORDER BY `t$typeId-1`.`id`, `t$typeId-1`.`count`";} 
	#if ($attr->typeId == 110 && $task == 'search' && $popular == 'on') print "<pre>query:" . str_replace('#__', 'jos_', $query) . "</pre>";
	...
	protected function _generateDerivedAttrs($attr, $table = '', $label = '', $summary = false) {
		...
		foreach ($attr->attrs as $a) {
			if ($a->flags & TableAttrOf::FLAG_INTERNAL) {
				continue;
			}
			$col = "a$a->attrOfId";
			if ($a->typeId > 99) {
		...
		$sql .= (func_num_args() == 1 ? ", $v.`count`, $i.`typeId`, $i.`published`, $i.`updated`, $i.`rating`" : '') . "\nFROM $tItem $i LEFT JOIN $tValue $v ON $i.`id` = $v.`itemId` "
			  . "WHERE $i.`typeId` = $attr->typeId "
			  . (JFactory::getUser()->get('id') == 0 ? "AND $i.`published` = 1 " : "AND $i.`published` = 1 ")
		...
		if (empty(self::$items[$typeId])) {
			...
			for ($i = 0; $i < count($results); $i++) {
				...
				foreach ($row as $label => $datum) {
					if (in_array($label, array('id', 'published', '_title', 'typeId','rating', 'updated')) || $datum === null) {
						continue;
					}
					$parts = explode('-', $label);
