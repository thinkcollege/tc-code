...
/**
 * $Id: xmap.php 168 2011-10-18 00:55:23Z guilleva $
 * $LastChangedDate: 2011-10-17 18:55:23 -0600 (Mon, 17 Oct 2011) $
...
$view = JRequest::getCmd('view', 'html', "REQUEST");
$task = JRequest::getCmd('task', '', "REQUEST");
$news = JRequest::getInt('news', 0, "REQUEST");

// Security check
$Itemid = JRequest::getInt('Itemid', 0, "REQUEST");
JRequest::setVar('Itemid', $Itemid);
if (!in_array($view,array('html','xml','xslfile','xsladminfile')) ) {
    $view = 'html';
}
...
if ($xSitemap->usecache) {
    $lang = JFactory::getLanguage();
    sendHeaders($view);
    $xmapCache->call('xmapCallShowSitemap', $view, $xSitemap->id, $excluded_items, $lang->getName(), $mainframe->getCfg('sef'), $user->get('id'), $news,$xSitemap->compress_xml);    // call plugin's handler function
} else {
 ...
 */
function xmapCallShowSitemap($view, $sitemapid, $excluded_items, $locale='', $sef='', $userid=0, $news=0,$compress=false)
{
    ...
    function printMenuTree(&$menu, &$cache, $extensions)
    {
        ...
        foreach ($items as $i => $item) {        // Add each menu entry to the root tree.
            ...
            if ($node->home) {
                ...
                $homeLink = $default->link;

                $languageString = '';
                $ItemidString = '';
                if (!empty($_REQUEST['lang'])) {
                  // is this default language ?
                  $lang = & JFactory::getLanguage();
                  $language = $lang->getTag();
                  $language = explode('-', $language);
                  $language = $language[0];
                  $conf =& JFactory::getConfig();
                  $jfDefaultLang = $conf->getValue( 'config.defaultlang');
                  if (!empty($jfDefaultLang) && $language != $jfDefaultLang) {
                    $langString = 'lang=' . JRequest::getCmd( 'lang');
                  }
                }

                $connector = strpos( $homeLink, '?') !== false ? '&' : '?';
                if (strpos( $homeLink, 'Itemid=') === false) {
                  $ItemidString = 'Itemid='.$node->id;
                }
                $languageString = empty( $ItemidString) || empty($languageString) ? $languageString : '&' . $languageString;
                $homeLink .= $connector . $ItemidString . $languageString;

                $node->link = JRoute::_( $homeLink);
