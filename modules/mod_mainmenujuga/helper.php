...
class modMainMenuJugaHelper
{
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			...
			$string = $cache->call(array('modMainMenuJugaHelper', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
	...
	function render(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			case 'list_flat' :
				...
				mosShowHFMenuJuga($params, 1);
				break;

			case 'horiz_flat' :
				// Include the legacy library file
				require_once(dirname(__FILE__).DS.'legacy.php');
				mosShowHFMenuJuga($params, 0);
				break;

			case 'vert_indent' :
				// Include the legacy library file
				require_once(dirname(__FILE__).DS.'legacy.php');
				mosShowVIMenuJuga($params);
				break;

			default :
				// Include the new menu class
				$xml = modMainMenuJugaHelper::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class JMenuJugaTree extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new JMenuJugaNode(0, 'ROOT');
	...
	function addNode(&$params, $item)
	{
		...
		$node = new JMenuJugaNode($item->id, $item->name, $item->access, $data);
 ...
 * @since		1.5
 */
class JMenuJugaNode extends JNode
{
	/**
	 * Node Title
	 */
