...
*/
function mosGetMenuJugaLink($mitem, $level = 0, & $params, $open = null)
{
	global $Itemid;
	$txt = '';
	...
	if (!$current_itemid) {
		...
		if ($current_itemid == $mitem->id) {
			...
			if ($params->get('activate_parent') && isset ($open) && in_array($mitem->id, $open)) {
				...
				if ($mitem->type == 'url' && ItemidContainedJuga($mitem->link, $current_itemid)) {
					$id = 'id="active_menu' . $params->get('class_sfx') . '"';
				} else {
					$id = '';
				}
...
*/
function mosShowVIMenuJuga(& $params)
{
	...
	$rows = $menu->getItems('menutype', $params->get('menutype'));
		...
		// set rows = filtered rows and continue		
		$rows = $jugaRows;
	...
	mosRecurseVIMenuJuga(0, 0, $children, $open, $indents, $params);
}
...
* hierarchial menu
*/
function mosRecurseVIMenuJuga($id, $level, & $children, & $open, & $indents, & $params)
{
	if (@ $children[$id]) {
		...
		foreach ($children[$id] as $row) {
			...
			$jugaRights = $dispatcher->trigger( 'checkRights', array( $juga_userid, $query, $juga_site ) );
			if ($jugaRights[0]->access == true) {
				echo mosGetMenuJugaLink($row, $level, $params, $open);
			}			

			// show menu with menu expanded - submenus visible
			if (!$params->get('expand_menu')) {
				if (in_array($row->id, $open)) {
					mosRecurseVIMenuJuga($row->id, $level +1, $children, $open, $indents, $params);
				}
			} else {
				mosRecurseVIMenuJuga($row->id, $level +1, $children, $open, $indents, $params);
			}
...
*/
function mosShowHFMenuJuga(& $params, $style = 0)
{
	$menu = & JSite::getMenu();
	$user = & JFactory::getUser();
	...
	if(is_array($rows) && count($rows)) {
		foreach ($rows as $row)
		{
			if ($row->access <= $user->get('aid', 0)) {
				...
				$jugaRights = $dispatcher->trigger( 'checkRights', array( $juga_userid, $query, $juga_site ) );
				if ($jugaRights[0]->access == true) {
					$links[] = mosGetMenuJugaLink($row, 0, $params);
				}			
			}
		}
	}
...
*/
function ItemidContainedJuga($link, $Itemid)
{
