var Uploader = new Class({
	...
	onSelect: function(file) {
		...
		if(this.filter.length && !this.filter.contains(this.fileinfo(file.name).ext)){
	...
	fileCreate: function(file) {
		...
		file.element = new Element('li').addClass('queue-text').addClass('file').addClass(f.ext).adopt(				
			text
		).adopt(
			status
		).adopt(
